...
$uniqid=$_POST['uniqid'];
  $input = "INSERT INTO tb_trx_p_plantation(id_trx, 
                                tgl_trx, 
                                raw_material,
                                pengepul, 
								date_create, 
    ...
    mysqli_query($konek, $input);
								...
								$query_sup  = "select * from tb_trx_p_plantation where uniqid_trx ='$uniqid'";
...
<div class='row'>
    ...
    <table id="user_data" class="table table-bordered table-stripe">
     ...
     <thead>
      <tr>
       <th size="50%">Plot</th>
       <th size="45%">Qty Basah</th>
	   <th size="45%">Qty Kering</th>
 ...
 $(document).ready(function(){
  ...
  function fetch_data()
  {
   var dataTable = $('#user_data').DataTable({
    ...
    "order" : [],
    "ajax" : {
     url:"halaman/hal_plantation/fetch.php?id=<?php echo $sup['id_trx']; ?>",
     type:"POST"
    }
   });
  }
  ...
  {
   $.ajax({
    url:"halaman/hal_plantation/update.php",
    method:"POST",
  ...
  $(document).on('click', '#insert', function(){
   ...
   var total1 = $('#data4').text();
   if(first_name != '' && last_name != '' && total != '' && total1 != '')
   {
    $.ajax({
     url:"halaman/hal_plantation/insert.php?id=<?php echo $sup['id_trx']; ?>",
     method:"POST",
     data:{first_name:first_name, last_name:last_name, total:total, total1:total1},
     success:function(data)
     {
      $('#alert_message').html('<div class="alert alert-success">'+data+'</div>');
      $('#user_data').DataTable().destroy();
  ...
  $(document).on('click', '.delete', function(){
   ...
   {
    $.ajax({
     url:"halaman/hal_plantation/delete.php",
     method:"POST",
     data:{id:id},
     success:function(data){
      $('#alert_message').html('<div class="alert alert-success">'+data+'</div>');
